...
const DANGEROUS_BUN_PROPERTIES = new Set([
  "file",
  "write",
  "spawn",
  "spawnSync",
...
const DANGEROUS_PROCESS_PROPERTIES = new Set([
  "env",
  "exit",
  "kill",
  "chdir",
...
function addDynamicModuleHits(content: string, hits: Set<string>): void {
  for (const match of content.matchAll(/\b(?:require|import)\s*\(\s*String\.fromCharCode\s*\(([^)]*)\)\s*\)/g)) {
  ...
  for (const match of content.matchAll(/\b(?:require|import)\s*\(([^;\n]{1,300})\)/g)) {
...
function addPropertyAccessHits(
  ...
  for (const match of content.matchAll(computedAccess)) {
    const property = decodeSimpleStringExpression(match[1]);
    if (property && properties.has(property)) hits.add(label);
  }
}

function addAliasPropertyHits(content: string, hits: Set<string>): void {
  const aliases = new Map<string, "Bun" | "process">();
  for (const match of content.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(Bun|process)\b/g)) {
  ...
  for (const [alias, source] of aliases) {
    addPropertyAccessHits(
      content,
      alias,
      source === "Bun" ? DANGEROUS_BUN_PROPERTIES : DANGEROUS_PROCESS_PROPERTIES,
      source === "Bun" ? "dangerous Bun system API usage" : "dangerous process API usage",
      hits
    );
  }
}

function addDestructuringHits(content: string, hits: Set<string>): void {
  for (const match of content.matchAll(/\b(?:const|let|var)\s*\{([^}]+)\}\s*=\s*Bun\b/g)) {
  ...
  for (const match of content.matchAll(/\b(?:const|let|var)\s*\{([^}]+)\}\s*=\s*process\b/g)) {
    for (const prop of match[1].split(",")) {
      const name = prop.trim().split(/\s*:/, 1)[0]?.trim();
 ...
 * against a malicious author using JS reflection, encoding, or dynamic import.
 */
export function detectDangerousBackendCapabilities(content: string): string[] {
  const hits = new Set<string>();
  const sources = new Set([content, normalizeJavaScriptForSafetyScan(content)]);
